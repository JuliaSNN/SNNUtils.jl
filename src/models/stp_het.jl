...
@rsubset! stp_data :fit_U .> 0.1 && :fit_U .< 0.9

dropmissing!(stp_data, :post_cell_class)
inh_inh_df = @rsubset stp_data String(:synapse_type) == "in" && String(:post_cell_class) == "in"
exc_inh_df = @rsubset stp_data String(:synapse_type) == "ex" && String(:post_cell_class) == "in"
exc_exc_df = @rsubset stp_data String(:synapse_type) == "ex" && String(:post_cell_class) == "ex"
inh_exc_df = @rsubset stp_data String(:synapse_type) == "in" && String(:post_cell_class) == "ex"
...
function  sample_stp_campagnola(N, type; df = stp_data, weights=true) 
    if type == :exc_exc
        return sample_stp_params(N; df = exc_exc_df, weights=weights)
    elseif type == :exc_inh
        return sample_stp_params(N; df = exc_inh_df, weights=weights)
    elseif type == :inh_exc
        return sample_stp_params(N; df = inh_exc_df, weights=weights)
    elseif type == :inh_inh
        return sample_stp_params(N; df = inh_inh_df, weights=weights)
    else
